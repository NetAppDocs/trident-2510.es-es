...
----
. Usa Helm para instalar Trident Protect. Reemplaza `<name-of-cluster>` por un nombre de clúster, que se asignará al clúster y se usará para identificar las copias de seguridad y las instantáneas del clúster:
+
[source, console]
----
...
NOTE: Para obtener el gráfico Helm, primero descarga el gráfico Helm en una máquina con acceso a internet usando  `helm pull trident-protect --version 100.2510.0 --repo https://netapp.github.io/trident-protect-helm-chart`, luego copia el archivo  `trident-protect-100.2510.0.tgz` resultante a tu entorno sin conexión e instala usando  `helm install trident-protect ./trident-protect-100.2510.0.tgz` en vez de la referencia del repositorio en el paso final.
