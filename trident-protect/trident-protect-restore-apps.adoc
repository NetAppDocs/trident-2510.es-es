...
* Cuando usas un CR para restaurar a un nuevo espacio de nombres, debes crear manualmente el espacio de nombres de destino antes de aplicar el CR. Trident Protect crea automáticamente espacios de nombres solo cuando usas la CLI.
...
+-------------+-----------+--------+-----------------+--------------------------+-------------+
----
. Restaura la aplicación en el clúster de destino usando el nombre AppVault y la ruta de archivo: nota: cuando uses un CR, asegúrate de que el espacio de nombres previsto para la restauración de la aplicación exista en el clúster de destino.
...
NOTE: Si tu aplicación usa varios espacios de nombres y estos espacios de nombres tienen PVCs con el mismo nombre, las operaciones de restauración de instantáneas (tanto in situ como a un nuevo espacio de nombres) no funcionarán correctamente. Todos los volúmenes restaurados tendrán los mismos datos en vez de los datos correctos para cada espacio de nombres. Usa la restauración de copias de seguridad en vez de la restauración de instantáneas, o actualiza a la versión 26.02 o posterior que soluciona este problema.
