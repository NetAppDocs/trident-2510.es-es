...
** Trident solo admite volúmenes SMB montados en pods que se ejecutan en nodos Windows. Consulta link:../trident-use/trident-fsx-storage-backend.html#prepare-to-provision-smb-volumes["Prepárate para aprovisionar volúmenes SMB"] para más detalles.
...
NOTE: Si estás especificando una función de IAM para Trident, entonces puedes omitir especificar los campos  `apiRegion`,  `apiKey` y  `secretKey` explícitamente para Trident. Para más información, consulta link:../trident-use/trident-fsx-examples.html["Opciones de configuración de FSx for ONTAP y ejemplos"].
