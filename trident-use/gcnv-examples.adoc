...
| `debugTraceFlags` | Indicadores de depuración para usar cuando estés resolviendo problemas. Ejemplo,  `{"api":false, "method":true}`. No uses esto a menos que estés resolviendo problemas y necesites un volcado detallado del registro. | null 
...
| `supportedTopologies` | Representa una lista de regiones y zonas compatibles con este backend. Para más información, consulta link:../trident-use/csi-topology.html["Usa la topología CSI"]. Por ejemplo:
`supportedTopologies:
...
| `snapshotDir` | Acceso al `.snapshot` directorio | "verdadero" para NFSv4 "falso" para NFSv3 
...
Para más información, consulta link:../trident-concepts/virtual-storage-pool.html["Pools virtuales"].
