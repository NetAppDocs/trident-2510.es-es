...
A partir de la versión 25.10, NetApp Trident es compatible con link:https://docs.netapp.com/us-en/ontap-afx/index.html["Sistema de almacenamiento NetApp AFX"^]. NetApp AFX storage systems difieren de otros sistemas ONTAP (ASA, AFF y FAS) en la implementación de su capa de almacenamiento.
...
----
. Prueba la autenticación usando el certificado generado. Reemplaza <ONTAP Management LIF> y <vserver name> con la dirección IP de Management LIF y el nombre de SVM. Debes asegurarte de que el LIF tenga su política de servicio configurada en  `default-data-management`.
+
[listing]
----
