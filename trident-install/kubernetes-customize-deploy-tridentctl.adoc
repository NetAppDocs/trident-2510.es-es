...
IMPORTANT: Al instalar Trident en un repositorio privado, si estás usando el  `--image-registry` switch para especificar la ubicación del repositorio, no uses  `/netapp/` en la ruta del repositorio. Por ejemplo: `./tridentctl install --image-registry <image-registry> -n <namespace>`
